...
void FreeCamera::Update(const SceneContext& sceneContext)
{
	...
	if (m_pCamera->IsActive())
	{
		...
		if (InputManager::IsMouseButton(InputState::down, VK_LBUTTON) &&
			InputManager::IsMouseButton(InputState::down, VK_RBUTTON))
		{
			const auto& mouseMove = InputManager::GetMouseMovement();
			if (move.z == 0) move.z = -static_cast<float>(mouseMove.y) / 2.f;

			mouseMoved = mouseMove.y != 0;
		}
		else if (InputManager::IsMouseButton(InputState::down, VK_LBUTTON))
		{
			const auto& mouseMove = InputManager::GetMouseMovement();
		...
		else if (InputManager::IsMouseButton(InputState::down, VK_RBUTTON))
		{
			const auto& mouseMove = InputManager::GetMouseMovement();
			look.x = static_cast<float>(mouseMove.x);
			if (move.y == 0) move.y = -static_cast<float>(mouseMove.y);
		...
		const auto up = XMLoadFloat3(&GetTransform()->GetUp());
		...
		currPos += up * move.z * currSpeed * elapsedTime;
